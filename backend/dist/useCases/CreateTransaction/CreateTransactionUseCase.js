...
class CreateTransactionUseCase {
    ...
    async execute(data) {
        ...
        while (minDate === undefined || null || "") {
            ...
            i++;
            if (i === data.length)
                throw new Error("Não há datas válidas");
        }
        // limpando os dados por valores faltantes, indefinidos, "" ou com data diferente da primeira transação
        const filteredData = data.filter((obj) => Object.values(obj).every((val) => val !== "" && val !== undefined && val !== null) &&
        ...
        const isValid = filteredData.every((obj) => Object.values(obj).every((val) => val !== "" || val !== undefined) &&
            obj.dataTransacao.getFullYear() === minDate.getFullYear() &&
            obj.dataTransacao.getMonth() === minDate.getMonth() &&
            obj.dataTransacao.getDate() === minDate.getDate());
        ...
        if (filteredData.length === 0)
            throw new Error("Após limpar o arquivo, não sobraram valores para serem salvos");
        const dataCadastro = new Date();
        const transactions = filteredData.map((transfer) => {
            return new Transaction_1.Transaction(transfer, dataCadastro);
        });
        // checa se já foi enviado um arquivo de transação para esta data
        const dbGroup = await this.transactionRepository.selectGroupedTransactions();
        const isSameDate = dbGroup.some((v) => v.dataTransacoes.getFullYear() === minDate.getFullYear() &&
